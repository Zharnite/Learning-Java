...
public class Picture extends SimplePicture 
{
  ...
  public void mirrorTemple()
  {
    ...
    for (int row = 27; row < 97; row++)
    {
      ...
      System.out.println(countOfLoop);
    }
  }
  ...
  public void mirrorGull() {
	  ...
	  for (int row = 0; row < height; row++) {
		  for (int col = 0; col < mirrorPoint; col++) {
			  int col2 = mirrorPoint - col + mirrorPoint;
			  if(col2 < width) {
				  leftPixel = pixels[row][col];      
			      rightPixel = pixels[row][col2];
			      rightPixel.setColor(leftPixel.getColor());
			  }
		  }
	  }
	  
	  int mirrorPoint2 = 350;
	  
	  for (int row = 230; row < 330; row++) {
		  for (int col = 230; col < mirrorPoint2; col++) {
	        leftPixel = pixels[row][col];      
  ...
  public void copy2(Picture fromPic, int startRow, int startCol, int copyStartRow, int copyEndRow, int copyStartCol, int copyEndCol) {
	  Pixel fromPixel = null;
	  Pixel toPixel = null;
	  Pixel[][] toPixels = this.getPixels2D();
	  Pixel[][] fromPixels = fromPic.getPixels2D();
	  //toRow  and toCol is where you start the image on the new canvas
	  //and that is startRow and startCol
	  //copyStartRow is start of picture and copyEndRow is end of picture to copy
	  for(int fromRow = copyStartRow, toRow = startRow; fromRow < fromPixels.length && toRow < toPixels.length; fromRow++, toRow++) {
